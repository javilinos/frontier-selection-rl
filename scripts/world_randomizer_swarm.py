...
def generate_obstacles(num_objects: int, drone_coords: list[dict[str, list[float]]],
    ...
    for i in range(num_objects):
        while True:
            ...
            too_close = any(distance((x, y), drone['xyz'][:-1]) <
                            min_distance for drone in drone_coords)
...
def generate_world(world_name: str, num_world: int, num_object: int,
                   safety_margin: int, num_drones: int) -> None:
    """Generate world"""
    ...
    for i in range(num_world):
        world_name = "world_swarm"
        drones = []
        j = 0
        while True:
            drone = randomize_drone_pose(upper=world_size, lower=-world_size)
            drone_xy = drone['xyz'][:-1]
            too_close = any(distance(drone_xy, drone['xyz'][:-1]) <
                            safety_margin for drone in drones)

            if not too_close:
                drone["model_name"] = f"drone{j}"
                j += 1
                drones.append(drone)
            if len(drones) == num_drones:
                break
        obstacles = generate_obstacles(
            num_object, drones, safety_margin, world_size, -world_size)
        ...
        print(f"World_swarm has been saved")
...
def main():
    ...
    parser.add_argument('number_of_drones', metavar='drones', type=int,
                        default=4, help='number of drones to generate')
    ...
    generate_world(world_name, num_world, num_obj, mos, num_drones)
