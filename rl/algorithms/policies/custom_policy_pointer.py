...
class ActorCriticPolicy(BasePolicy):
    ...
    def _build_pointer_extractor(self) -> None:

        self.pointer_extractor = PointerExtractor(
            self.features_dim,
            net_arch=self.net_arch,
            activation_fn=self.activation_fn,
            device=self.device,
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        self.value_net = nn.Linear(self.pointer_extractor.latent_dim_vf, 1)
        ...
        if self.ortho_init:
            ...
            module_gains = {
                ...
                self.pointer_extractor.encoder: np.sqrt(2),
                self.pointer_extractor.actor_mlp: np.sqrt(2),
                self.pointer_extractor.critic_encoder: np.sqrt(2),
                self.pointer_extractor.critic_mlp: np.sqrt(2),
                self.pointer_extractor.actor_score: 0.01,  # Lower gain for final output layer
    ...
    def forward(self, obs: th.Tensor, frontier_features: List[th.Tensor], deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        if self.share_features_extractor:
            logits_pi, latent_vf = self.pointer_extractor([features])
    ...
    def evaluate_actions(self, obs: PyTorchObs, frontier_features_batch: List[List[th.Tensor]], actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        ...
        for i, frontier_features in enumerate(frontier_features_batch):
            ...
            logits_pi = self.pointer_extractor.forward_actor([features])
        ...
        latent_vf = self.pointer_extractor.forward_critic(frontier_features_batched)
        values = self.value_net(latent_vf)
    ...
    def predict_values(self, obs: PyTorchObs, frontier_features: List[th.Tensor]) -> th.Tensor:
        ...
        latent_vf = self.pointer_extractor.forward_critic([features])
